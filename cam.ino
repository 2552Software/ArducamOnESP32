...
void serverStream(){
  ...
  while (1){
    ...
    myCAM.set_fifo_burst();
    if (!client.connected()) {
      Serial.println(F("!client.connected() break"));
      break;
    }
    response = "--frame\r\n";
    response += "Content-Type: image/jpeg\r\n\r\n";
    ...
    while ( len-- )
    {
      ...
      if ( (temp == 0xD9) && (temp_last == 0xFF) ) //If find the end ,break while,
      {
        ...
        myCAM.CS_HIGH();; 
        if (!client.connected()) {
          Serial.println(F("!client.connected() 2 break"));
          break;
        }
        client.write(&buffer[0], i);
        is_header = false;
      ...
      { 
        ...
        else
        {
          ...
          myCAM.CS_HIGH(); 
          if (!client.connected()) {
            Serial.println(F("!client.connected() 3 break"));
            break;
          }
          client.write(&buffer[0], bufferSize);
          i = 0;
  ...
  if (!client.connected()) {
    Serial.println(F("!client.connected() 4 break"));
    break;
  }
 } // while
}
...
void setup() {
    ...
    Serial.print(F("set the CS as an output: CS: "));
    Serial.println(CS);
    ...
    Serial.println(F("Check if the ArduCAM SPI bus is OK"));
    while(1){
      delay(500);
      myCAM.write_reg(0, 0x55);
      temp = myCAM.read_reg(0);
      Serial.println(temp);
      if (temp != 0x55){
        Serial.println(F("SPI1 interface Error, try again"));
      }
      else {
        break;
      }
    ...
    Serial.println(F("Check if the camera module type is OV2640"));
